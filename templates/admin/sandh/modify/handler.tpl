...
{formtextinput textMode="hidden" id="handler_name"}




     <div class="z-admin-content-pagetitle">
        <h1><i class="mediasharex-icon-folder"> </i> {gt text="Modify handler settings"}</h1>
    </div>   
    ...
    <div>
          <h3>{gt text="Hanlder info"}</h3>
            <h4 class="z-formrow" >{$handler_info.name}</h4>
            <div class="z-formrow" ><strong>{gt text="Version"}:</strong>  {$handler_info.version} <strong> {gt text="Author"}:</strong>  {$handler_info.author}</div>         
    </div>
    <div>
